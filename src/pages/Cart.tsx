...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    try {
      setSubmitting(true);
      const res = await cartService.checkout(items);
      toast({ title: "Order placed", description: `Order ID: ${res.orderId}` });
      clear();
    } catch {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <main className="container mx-auto px-4 py-12 flex-1 animate-fade-in">
        ...
        {items.length === 0 ? (
          ...
          <div className="grid lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 divide-y divide-border border-y border-border">
              {items.map((i) => (
                <div key={i.id} className="flex gap-4 py-6 animate-fade-in">
            ...
            <aside className="bg-secondary/40 p-6 h-fit animate-scale-in">
              ...
              <Button className="w-full mt-4" onClick={handleCheckout} disabled={submitting}>
                {submitting ? (<><Spinner className="h-4 w-4 mr-2 text-primary-foreground" /> Processing...</>) : "Checkout"}
              </Button>
